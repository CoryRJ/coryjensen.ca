...
Nn::Nn(int numOfLayers, int *layers)
{
	act = relu;
	act_dir = relu_dir;
	error = mse;
...
float Nn::softmax_dir(float inp)
{
...
void Nn::backprop(float *real_vals)
{
	batch++;
...
void Nn::setAct(float (*f)(float),float (*f_d)(float))
{
	act = f;
	act_dir = f_d;
}
void Nn::setError(float (*f)(float,float,float),float (*f_d)(float,float,float))
{
	error = f;
	error_dir = f_d;
}

void Nn::run(float *input)
{
	node *aNode = nodes[0];
	weight *aWeight;
...
void Nn::update()
{
	...
	for(int i = numLayers-2; i > -1; i--)
	{
		...
		while(aWeight != NULL)
		{
			if(adam)
			{
	/*			if(signbit(aWeight->d) ^ signbit(aWeight->d_m))
				{
					aWeight->d_m = 0;
					aWeight->d_v = 0;
				}*/
		...
		while(aNode != NULL)
		{
			...
			if(adam)
			{
			/*	if(signbit(aNode->bias_d) ^ signbit(aNode->bias_d_m))
				{
					aNode->bias_d_m = 0;
					aNode->bias_d_v = 0;
				}*/
